...
@Autonomous(name = "Auto Only Park", group = "Autonomous", preselectTeleOp = "Competition TeleOp")
public class AutoOnlyPark extends LinearOpMode {
    ...
    public static START_POSITION startPosition = START_POSITION.RED_RIGHT;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (!isStopRequested() && !opModeIsActive()) {
            ...
            telemetry.clearAll();
            telemetry.addData("Start FTC Wires (ftcwires.org) Autonomous Mode adopted for Team", "21233");
            telemetry.addData("---------------------------------------", "");
        ...
        if (opModeIsActive() && !isStopRequested()) {
            ...
            //driveTrain.arm.setSlidePosition(1000);
            //driveTrain.arm.update();
    ...
    public void buildParking() {
        ...
        midWayPose = new Pose2d(midX, 36, Math.toRadians(0)); //Choose the pose to move forward towards signal cone
        switch (vision.identifiedparkingLocation) {
            case 1:
                parkPose = new Pose2d(midX, 60, Math.toRadians(0));
                break; // Location 1
            case 2:
                parkPose = new Pose2d(midX, 36, Math.toRadians(0));
                break; // Location 2
            case 3:
                parkPose = new Pose2d(midX, 11, Math.toRadians(0));
                break; // Location 3
        }
        //break;
            /*
    ...
    public void selectStartingPosition() {
        ...
        while (!isStopRequested()) {
            telemetry.addData("Initializing FTC Wires (ftcwires.org) AutoOnlyPark adopted for Team:", "21233");
            telemetry.addData("---------------------------------------", "");
